...
If you wish to add the attention module seperately, you can do so using the `AttentionAugmentation` layer as well.

```python
from attn_augconv import AttentionAugmentation

ip = Input(...)

# make sure that input to the AttentionAugmentation layer has (2 * depth_k + depth_v) filters.
x = Conv2D(2 * depth_k + depth_v, ...)(ip)
x = AttentionAugmentation(depth_k, depth_v, num_heads)(x)
...
```
